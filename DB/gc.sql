...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 16-08-2017 a las 18:18:53
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_editarFichaproyecto` (IN `id` INT, IN `nom` VARCHAR(50), IN `obje` VARCHAR(100), IN `version` VARCHAR(5), IN `urls` VARCHAR(50), IN `cliente` INT, IN `esta` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertarFichaproyecto` (IN `nomb` VARCHAR(20), IN `obje` VARCHAR(100), IN `url` VARCHAR(50), IN `version` VARCHAR(15), IN `id_cliente` INT, IN `estado` INT)  NO SQL
BEGIN
INSERT INTO tbl_fichaproyecto (id_ficha, titulo, obj_general, url, version, id_cliente, estado, observacion) VALUES (NULL, nomb, obje, url, version, id_cliente, estado);
END$$
...
INSERT INTO `tbl_fichaproyecto` (`id_ficha`, `titulo`, `obj_general`, `Url`, `version`, `observacion`, `id_cliente`, `estado`) VALUES
(3, 'Nival2', 'Testing', '2', '2.0', '', 1, 1),
(4, 'Eribel', 'Testing', '0', '2', '', 1, 1),
(5, 'ASUS', 'Vender', '0', '2', '', 1, 1),
(7, 'petmate', 'veterinaria', './uploads/Bitacora 12.xls', '3', 'jj', 1, 3),
(8, 'CountNumb', 'Apoyar los procesos de contabilidad de la empresa', './uploads/gc (1).sql', '1', 'No se considera viable el proyecto', 1, 4),
(9, 'Agiles', 'Facilitar procesos', './uploads/ingenieria_sistemas_1 (1).pdf', '1.2', 'hh', 1, 2);
